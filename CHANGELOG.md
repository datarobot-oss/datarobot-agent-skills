...
- `datarobot-agent-assist`: The dress rehearsal keeps its cross-provider guard when a spec names a model that is close but not exact. It read the provider off the front of the request without stripping the `datarobot/` prefix, so every prefixed request looked like provider "datarobot", matched nothing, and fell through to an arbitrary first-in-catalog pick. A spec asking for a Claude model could rehearse against an Azure one.
- `datarobot-agent-assist`: `setup_template.py` checks the model against the catalog over a direct API call rather than through `dr llm-gateway list`, which falls back to its own stored profile when the credentials it is handed do not verify and so could answer about a different DataRobot instance. It also refuses a model name carrying characters that would break the `.env` line it is written into, and treats an instance whose LLM Gateway is switched off as one to pick a deployed LLM on instead of failing later.
