...
- `packages/datarobot-skills-utils`: New shared Python package (stdlib-only) for the opencode runtime used by `datarobot-gap-analysis` and the `datarobot-agent-assist` swarm simulation: private `dr opencode serve` lifecycle (free port, isolated git workdir, process-group shutdown so no servers leak), worker invocation with the anti-tool preamble and argv sanitization, and JSONL event parsing with token/cost accounting. Published to PyPI on every GitHub release by `publish-pypi.yml` (OIDC trusted publishing), versioned independently of the plugin. Skills resolve it from a sibling `packages/` checkout, an installed copy, or a one-time install from PyPI, so copies made by the universal installer or Antigravity work without a manual step.
