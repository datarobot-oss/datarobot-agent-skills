...
### Added
- `datarobot-gap-analysis`: Regulatory findings carry structured remediation (prerequisite, ordered steps, DataRobot docs link) sourced from the mitigation catalog, and the report opens with a Path to Compliance section that groups the unsatisfied mitigations by what unblocks them (deploy through DataRobot, Pulumi settings, LLM test-suite runs, console work). Layer 4 names the active IaC configuration variant (symlink-selected `infra/infra/<area>.py`) in its note and to the judge. The HTML report uses the dr-ui design tokens (alpine-light / midnight-gray palettes, Inter and Roboto Mono, semantic success / warning / destructive colors).
- `datarobot-gap-analysis`: Every regulatory finding carries a structured fix: prerequisite (which Pulumi resource it attaches to and whether the repo has it), numbered steps, and a DataRobot docs link resolved at run time from `docs.datarobot.com/llms.txt` (the catalog names a `docs_topic`, never a pinned URL). The coverage table and the compliance path are one section: each gap expands to its evidence and fix, grouped by the step that closes it, and POL findings no longer repeat in the Findings list. The posture and re-platform advice now know whether the repo already builds on af-components (from `.datarobot/answers` / `.copier-answers.yml`) and which agent framework it uses (the template's flavors are read from the `af-component-agent` Copier template at run time), so an af-components repo is told to add a DataRobot agent deployment rather than re-generate, and a non-native framework (pydantic-ai, deepagents, ...) is routed to the Base agent template instead of a rewrite. Layer 4 states the active IaC configuration variant it assessed. The HTML report uses the dr-ui design tokens (alpine-light / midnight-gray palettes, Inter and Roboto Mono, semantic state colors) in light and dark mode, and declares `darkreader-lock` so browser dark-mode extensions leave the report's own palette alone.
...
- `packages/datarobot-skills-utils`: Published to PyPI on every GitHub release by the new `publish-pypi.yml` workflow (OIDC trusted publishing), versioned independently of the plugin and skipped when the version is already on PyPI. The `_bootstrap.py` shim in `datarobot-gap-analysis` and `datarobot-agent-assist` resolves the package in order: a sibling `packages/` checkout, an installed copy, then a one-time install from PyPI, so skills copied without `packages/` (universal installer, Antigravity) work with no manual step and a repo checkout keeps using local sources.

### Fixed

- `datarobot-gap-analysis`: DataRobot's own templates no longer score RE-PLATFORM. Regulatory mitigations that cannot be assessed (no LLM, or no code evidence) stay in the coverage table as "not assessed" instead of counting as high-severity structural gaps; `**/` globs match root-level paths such as `infra/`, so Pulumi evidence and Layer-2 files are found; test fixtures, minified bundles, structured values and word-like labels such as `first-user-api-key` are no longer reported as secrets (the generic rule now requires a credential-shaped value: mixed character classes or a long hex/base64 run); Dockerfile `ARG` defaults are resolved and devcontainer/stage-alias `FROM`s are skipped before the base-image allowlist check; models served by the org's LLM Gateway (`dr llm-gateway list`) and the gateway provider families count as approved; `pip-audit` audits `uv.lock` exports (in a temp dir, in parallel) rather than the running interpreter, reports one finding per vulnerable package, and skips advisories waived in `trivy-ignore.rego`; the Pulumi program is found by shape (`infra/`, `__main__.py`, `datarobot_pulumi_utils` imports) instead of the first 200 `.py` files in directory order, and LLM judges read source files matched by named globs before config and never lockfiles or coverage output, so Layer 4 sees the actual IaC and system prompts rather than stack YAML; `--out` accepts a directory and the HTML report lands next to the Markdown one instead of the current directory; the CLI exit code reflects the post-fix re-score when `--verify` runs. Repo-wide Layer 2 questions (tracing, logging, guardrails, cost controls, resilience, RBAC) report once with every location listed instead of once per file, and test, fixture, IaC, migration and CI files are no longer judged for runtime behaviour, by the LLM layer or by the semgrep injection rules. When no LLM client is available the report names what enables Layers 2 and 4 instead of a one-line skip. Model replies with trailing commentary parse, worker timeouts no longer echo the whole prompt into the report, and the skipped section is compact and collapsible. Docs no longer advertise `detect-secrets` (the secret scan is built in) or a non-existent guided-extraction CLI path; the re-platform hand-off is the `datarobot-agent-assist` skill.
