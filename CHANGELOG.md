...
- `datarobot-agent-assist`: Support DataRobot-deployed LLMs, not just LLM Gateway models. `list_llm_models.py` now lists both sources with a `source` discriminator and `deployment_id`, model selection recommends a deployed model when the gateway is empty, `agent_spec.md` gains `llm_deployment_id`, and `setup_template.py --llm-deployment-id` writes the deployed-model `.env` keys (`LLM_DEPLOYMENT_ID`, `INFRA_ENABLE_LLM=deployed_llm.py`, `USE_DATAROBOT_LLM_GATEWAY=0`) so the template routes to the deployment. `setup_template.py` now rejects the deployed-model name when no deployment id is given, instead of letting the run fail later at `pulumi up` with a misleading catalog error. `list_llm_models.py` forwards the CLI's log lines to stderr so a listing that came from a different DataRobot instance than the project targets is visible rather than silent.
