...
### Added
- `datarobot-gap-analysis`: New skill. Scores any agent repository (a GitHub URL or local path, DataRobot-built or not) against an enterprise-readiness framework of seven risk pillars (Security, Identity, AI Governance, Reliability, Ops, IT Conformance, Regulatory & Policy) across four layers: deterministic scanners (built-in secret scan with credential-shape checks; `trivy` for dependency CVEs, secrets, IaC misconfiguration and dependency licenses when installed, honoring `trivy-ignore.rego`, with `pip-audit` over `uv.lock` exports and `npm audit` as the fallback; `gitleaks` for secrets in git history; `hadolint` for Dockerfiles; `semgrep` routed by rule family; and the gates DataRobot application templates ship with: lint/type-check gate, lockfiles, Dependabot/Renovate, vulnerability scanning in CI, CODEOWNERS, pinned GitHub Actions), LLM code reasoning through parallel `dr opencode` workers (repo-wide questions such as tracing or guardrails report once; test, IaC, migration and CI files are not judged for runtime behaviour), policy conformance (Python floor across every component, library and license rules, base images with Dockerfile `ARG` resolution, models approved by the org's LLM Gateway catalog), and a regulatory layer sourced live from the org's DataRobot risk-management policy (EU AI Act by default). Every regulatory gap carries a prerequisite, numbered fix steps and a docs link resolved at run time from `docs.datarobot.com/llms.txt`, grouped into the ordered steps that close them. The Patch/Hybrid/Re-platform posture knows whether the repo already builds on af-components and which agent framework it uses, with the DataRobot agent template's flavors read from the `af-component-agent` Copier template, so it proposes a DataRobot agent deployment rather than a rewrite and never changes the framework unless asked. `--fix` applies safe fixes on a `gap-fixes/*` branch (secrets to env vars, CVE pins written to `pyproject.toml` and `uv.lock` regenerated with `uv lock`, Python version pins, CI and test scaffolds) and `--verify` re-scores. Reports are Markdown and HTML; the HTML uses the dr-ui design tokens in light and dark mode.
- `datarobot-agent-assist`: Mention `datarobot-gap-analysis` as a follow-up step after a successful deploy. Layer-2 findings are verified by a second LLM pass before they are reported or fixed, with repo-wide evidence hints and line-accurate citations; the secret scanner's findings get the same pass, so a resource URN or type name that merely contains "credential" is dropped and an unconfirmed secret is never auto-fixed; files over 48 KB reach the model as excerpts (header, definitions, windows around hint hits, real line numbers) rather than head-truncated blobs, timed-out checks are listed with the `--select ... --llm-timeout` command that reruns them, and the per-call default is 600 s; the shared package also raises each gateway model's output-token limit from the 8k the DataRobot opencode config declares to what the family supports, which is what cut long replies mid-JSON; reports open with a coverage summary of what each layer did and did not assess and close with LLM Gateway token usage per phase. Deterministic fixes reuse the saved `gap-findings.json` (`--fix --from`) instead of re-analyzing, and every other finding ships a copy-ready prompt for the developer's own coding agent in place of a one-shot LLM edit.
- `packages/datarobot-skills-utils`: New shared Python package (stdlib-only) for the opencode runtime used by `datarobot-gap-analysis` and the `datarobot-agent-assist` swarm simulation: private `dr opencode serve` lifecycle (free port, isolated git workdir, process-group shutdown so no servers leak), worker invocation with the anti-tool preamble and argv sanitization, JSONL event parsing with token/cost accounting, and reasoning-effort injection (`OpenCodeServer(models, reasoning="max")`) that asks each gateway-served model for the highest effort its provider accepts. Published to PyPI on every GitHub release by `publish-pypi.yml` (OIDC trusted publishing), versioned independently of the plugin. Skills resolve it from a sibling `packages/` checkout, an installed copy, or a one-time install from PyPI, so copies made by the universal installer or Antigravity work without a manual step.
