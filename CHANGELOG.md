...
- `datarobot-agent-assist`: Mention `datarobot-gap-analysis` as a follow-up step after a successful deploy. Layer-2 findings are verified by a second LLM pass before they are reported or fixed, with repo-wide evidence hints and line-accurate citations; reports open with a coverage summary of what each layer did and did not assess.
