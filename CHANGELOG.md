...
Repo-wide audit of every skill against `datarobot` 3.18.0/3.19.0 (static introspection plus live
read-only API verification), hardened through four rounds of behavioral testing with recorded
agent runs and a reviewer-prompted adversarial sweep that executed the PR's new behavior against
every deployment, workload, and project in a live org. 100+ verified defects fixed across 12
skills — SDK signature mismatches, nonexistent attributes/methods, deprecated APIs, semantic bugs
that returned wrong answers without raising, and doc gaps that misled agents in recorded runs.

### Fixed
- `datarobot-predictions`: `dr.Model.get(single-arg)` and nonexistent `Model.get_features()` replaced by `deployment.get_features()` (plain dicts; documented return shape and that `importance` is a model-independent association score, not Feature Impact); `model.target_name`/`target_type`/`project_name` reads replaced by `deployment.model` lookups with `.get()` guards (keys vary by deployment type); batch scoring uses `BatchPredictionJob.score(deployment, ...)` — with a `localFile` output path it blocks and writes the file (`download_timeout=120` caveat documented), replacing a broken monitor/download block; time-series info via `dr.DatetimePartitioning.get` (with retrained-`(actual)` column names mapped back to real input columns) instead of nonexistent `Project.get_time_series_info()`; the template script fills categoricals with real training levels via `Feature.get(...).get_histogram()` (histogram pseudo-buckets `=All Other=`/`==Missing==` filtered and surfaced as header notes), numerics with training medians/ranges, Booleans/Percentages from stats, Date samples in each feature's own `date_format`, and handles deleted-training-project, empty-feature, and metadata-unavailable deployments cleanly; the validator can now actually validate the generator's output (skips `#` metadata lines, survives ragged rows, treats `importance=None` as unknown); corrected `max_explanations=0` SHAP semantics and the `DATAROBOT_ENDPOINT` `/api/v2` suffix.
- `datarobot-model-training`: two-arg `dr.Model.get`; `Dataset.create_from_file` `name=` kwarg and positional-name form removed (upload then `dataset.modify(name=...)`); time-series pattern rebuilt on `dr.DatetimePartitioningSpecification` + `analyze_and_model(partitioning_method=...)` (replacing deprecated `set_target`, nonexistent `dr.PARTITIONING_METHOD` and fake kwargs); removed broken `project.start(...)` and nonexistent `wait_for_completion()` → `wait_for_autopilot()`; noted blocking `max_wait` semantics.
- `datarobot-data-preparation`: `create_from_file` signature corrected (create-then-rename recipe, script CLI unchanged, `max_wait` exposed); nonexistent `dataset.column_count` → `len(dataset.get_all_features())`; Quick Start's undefined helpers replaced with real operations; `target_leakage` no longer implied to exist on catalog datasets (project-features-after-EDA2 only).
- `datarobot-model-deployment`: deprecated `create_from_learning_model` replaced with the registered-model flow incl. `default_prediction_server_id` (required on DataRobot Cloud despite `Optional` typing) and the serverless `prediction_environment_id` alternative; dict-comparing metrics ranking → `ModelRecommendation.get(...).get_model()` (404s on unsupervised/unmodeled projects noted); `Deployment.list()`/`perform_model_replace(...)` corrected; undefined Quick Start helpers replaced.
- `datarobot-model-monitoring`: monitoring windows truncate `end_time` forward to the next hour top (back-truncation silently zeroed the in-progress hour: a report answered 0 predictions for a window that served 60); raw `targetDrift` REST workaround for the SDK `segment_value` bug (still present in 3.19.0) with real `start`/`end` params; `get_service_stats_over_time` bucket shape; median (not mean) response time; ROC via non-deprecated `insights.RocCurve.get`; drift example threshold aligned with the skill's own 0.15/0.3 table.
- `datarobot-feature-engineering`: undefined Quick Start helpers replaced with real calls; `get_or_request_feature_impact()` over bare `get_feature_impact()` (404 trap); `impactNormalized`/`impactUnnormalized` defined (not a share of signal); Pattern 3 (feature effects / partial dependence) added with an explicit output rule — reports that did not compute feature effects must not claim direction, and impact scores must not be phrased as shares/proportions.
- `datarobot-model-explainability`: `compute_shap_matrix.py` no longer crashes formatting `base_value` (single-element list); XEMP reference corrected (delete usage, `create_on_training_data` signature, row field names); `ShapImpact.create` `source="training"` default vs `"validation"` elsewhere documented; aggregated SHAP noted as unsigned; DataSlice filter `values` must be a list.
- `datarobot-external-agent-monitoring`: install covers the OTel packages the verify script imports; Step 4 installs the target project's own declared dependencies before running it; real `HTTPXClientInstrumentor`/llamaindex import paths; google-adk exporter kwarg; pydantic-ai logfire extra and `InstrumentationSettings` versions corrected.
- `datarobot-workload-api`: diagnose flow rebuilt on real response shapes (no `statusDetails.logTail`; events carry `eventType`) and bounded to the current proton's lifetime; OTel trace `duration` is milliseconds and span `duration` nanoseconds (previous units off by 1000× each way); `wait_for_replacement.py` matches the real `ReplacementStatus` enum (`errored`/`cancelled` — `failed` does not exist, so failures previously reported "completed"); autoscaling `minReplicaCount`/`maxReplicaCount`; metric-unit conversions; build `CANCELLED` terminal; `codeRef` under `imageBuildConfig`; spec lookups pointed at the docs-site OpenAPI; credentials guidance via `CredentialsBody`.
- `datarobot-discover`: install command fixed to the working repo-path form; catalog entries listed by `displayName`.
- All SDK skills: install one-liner `python -m pip install ... || uv pip install ...` with the installer decision stated in prose (bare `pip` failed in recorded runs: no pip in uv venvs, PEP 668).
- `datarobot-app-framework-cicd`: Simple Path workflows set `DATAROBOT_ENDPOINT` (SDK 3.x has no default endpoint — token-only CI runners failed `pulumi up`); setup docs cover the optional repository Variable.
