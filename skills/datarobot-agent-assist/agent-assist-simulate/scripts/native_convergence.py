...
def initialize(
    ...
    resolved_convergence_dir = _resolve_project_output(
...
def advance(spec_path: Path, convergence_dir: Path) -> ConvergencePreparation:
    ...
    if Path(state.convergence_dir) != resolved_convergence_dir:
        raise ValueError("convergence state directory does not match requested directory")
    ...
    state.expected_tasks = [
        task for task in tasks if isinstance(task, ConvergenceTask)
    ]
    write_json(state_path, state.model_dump(mode="json"))
    return ConvergencePreparation(
        status=state.status,
...
def report(
    ...
    resolved_output = _resolve_project_output(
    ...
    for task in diagnoser_tasks:
        try:
            ...
            if result is None or result.status != "exhausted":
                raise ValueError("diagnoser task does not reference an exhausted scenario")
            if result.structural_diagnosis is not None:
                raise ValueError("exhausted scenario already has a structural diagnosis")
    ...
    for task, proposal in validated:
        ...
        patch_records.append(
            PromptPatchRecord(
                ...
                prompt_hash_before=_prompt_hash(prompt_before),
                prompt_hash_after=_prompt_hash(current_prompt),
            )
        )
    ...
    for scenario_id, run_dir_text in state.rerun_dirs.items():
        ...
        if result_path.is_file():
            try:
                ...
                if result.scenario.model_dump(
                    mode="json"
                ) != current_by_id[scenario_id].scenario.model_dump(mode="json"):
                    raise ValueError("rerun scenario differs from confirmed criteria")
                rerun_results[scenario_id] = result
            except (OSError, ValueError, ValidationError) as exc:
                failures.append(f"{scenario_id}: invalid rerun result: {_one_line(exc)}")
...
def _resolve_project_output(project_root: Path, path: Path, label: str) -> Path:
    return _resolve_under_root(project_root, path, label)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    report_parser.add_argument(
        "--output", type=Path, default=Path("eval_report.md")
    )
    return parser
