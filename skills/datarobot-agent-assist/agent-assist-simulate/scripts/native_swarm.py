...
"""Prepare and aggregate a harness-orchestrated native simulation swarm."""
...
def prepare(
    ...
    resolved_runs_dir = _resolve_project_output(
        project_root, runs_dir, "swarm runs directory"
    )

    config, warnings = load_native_config(resolved_config)
    if config.execution.mode != "simulated":
        raise ValueError("selective_e2e execution is unavailable until Milestone 8")
