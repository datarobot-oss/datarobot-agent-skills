...
|---|---|---|---|
| 1 - Deterministic | A built-in secret scanner, `pip-audit` (over `uv.lock` exports and requirements files), `npm audit`, `semgrep`, plus presence checks: tests, CI, lint/type-check gate, lockfiles, dependency updates, vulnerability scanning, CODEOWNERS, action pinning | 13 | Optional scanners that aren't installed are skipped and named in the report. Layer 1 always runs. |
