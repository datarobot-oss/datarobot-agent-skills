...
|---|---|---|---|
| 1 - Deterministic | A built-in secret scanner (its findings get the same second-look verification pass as Layer 2 when an LLM is available, so a resource URN or type name that merely contains "credential" is dropped, and an unconfirmed finding is never auto-fixed), `trivy` (dependency CVEs, secrets, IaC misconfiguration, dependency licenses) with `pip-audit` and `npm audit` as the fallback, `gitleaks` for secrets in git history, `hadolint` for Dockerfiles, `semgrep`, plus presence checks: tests, CI, lint/type-check gate, lockfiles, dependency updates, vulnerability scanning, CODEOWNERS, action pinning | 15 | Optional scanners that aren't installed are skipped and named in the report. Layer 1 always runs. |
| 2 - LLM reasoning | A per-condition prompt template (under `scripts/prompts/`) reads the condition's `files_glob` and reasons about the code. Files are shown with numbered lines and each finding's quoted evidence is snapped onto the line that holds it. A file over 48 KB goes in as an excerpt (header, definition lines, and windows around its `hint_patterns` hits, with omitted ranges marked and real line numbers) instead of a head-truncated blob. Checks that time out are listed in the coverage block with the `--select ... --llm-timeout` command that reruns just those. Per-condition `hint_patterns` are grepped across the whole repo and shown to the model as evidence outside its file cap. Every finding then gets a second verification call that re-reads the cited region and refutes, weakens or confirms it (`--no-verify` disables it); refuted findings are dropped and named in Engine Notes, and only verified findings may be auto-fixed. Locations sharing a `root_cause` merge into one finding. Relational checks (`SEC-001`, `IDN-003`) require *both* file groups involved, or are marked skipped-with-reason, never guessed. When the Pulumi program deploys the repo as a DataRobot custom application, every prompt is told that identity headers (`x-user-id`, `x-datarobot-api-key`, ...) are set by the platform proxy, so reading them is not flagged as trusting client input. | 20 | Skipped, with a stated reason, if no LLM client is configured. |
