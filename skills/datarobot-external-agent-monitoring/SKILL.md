...
5. Check if the `datarobot` Python SDK is available. If not, install it together with the OTel packages the Step 5 verify script imports: `python -m pip install datarobot opentelemetry-sdk opentelemetry-api opentelemetry-exporter-otlp-proto-http || uv pip install datarobot opentelemetry-sdk opentelemetry-api opentelemetry-exporter-otlp-proto-http` (the `uv pip` fallback fires automatically when the environment has no pip — uv-created venvs, PEP 668 systems). When rewriting this command — e.g. adding `-r requirements.txt` in Step 4 — decide the installer first: if `python -c "import pip"` fails, go straight to `uv pip install`.
...
1. **Add dependencies** to the project's dependency file:
   ...
   - Then **install the project's full dependency set** — its own declared dependencies plus the
     additions — before running it (`python -m pip install -r requirements.txt || uv pip install -r requirements.txt`,
