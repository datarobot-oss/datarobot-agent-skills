...
def generate_prediction_data_template(
    ...
    writer = csv.DictWriter(
        output, 
        fieldnames=[f.name for f in prediction_features]
    )
    writer.writeheader()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python generate_prediction_data_template.py <deployment_id> [n_rows] [output_file]", file=sys.stderr)
        sys.exit(1)
